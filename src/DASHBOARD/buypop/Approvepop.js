...
// await this.setState({admindetail: sanaa});
    ...
    async SubmitDataapprove (e) {
        ...
        const transaction = await LandAbi.methods.makeApproved(this.state.Landid, this.props.click2).send({
          from : Acc[0] });
         if(!transaction){
           ...
           socket.emit("Approved", [ {approvedUser:this.props.click2 ,
            whoapprove: Acc[0],
            Landid: this.state.Landid,
            time:  new Date()
            }]);

            socket.on("SendApprovedata",(data)=>{console.log("SendApproverdata", data)});
...
<Modal show={this.state.showco}   className="setLogin" >
        <Modal.Header bsPrefix="modal-header " className="d-block" >
         <Modal.Title ><MdPerson className="signup2" /> Approve Buyer</Modal.Title>
         </Modal.Header>
         <Modal.Body>
  ...
  <Button variant="primary"  className="chkbtn" type="submit" >
    Submit
         ...
         <Modal.Footer>
           <Button variant="secondary" onClick={()=>{this.props.click(); this.props.click3();}} className="chkbtn">
             Close
           </Button>
